...
public class JobForMonth extends QuartzJobBean {
    ...
    protected void executeInternal(JobExecutionContext jobExecutionContext){
        ...
        String endTime = DateFormatUtils.format(now,"yyyy-MM") + "-01 00:00:00";
        now.add(Calendar.MONTH, -1);
        String beginTime = DateFormatUtils.format(now,"yyyy-MM") + "-01 00:00:00";
    ...
    public void execute(String beginTime,String endTime){
        ...
        List<List<ScriptVO>> ktrs = StringUtil.deepCopy(EnvConfig.CONSTANT_SCRIPTVO_LIST_KTRS_MONTH);
        ktrs.forEach(list -> list.forEach(scriptVO -> scriptVO.setEtlEnv(etlEnv)));
        job.etlData(ktrs);

        List<List<ScriptVO>> kjbs = StringUtil.deepCopy(EnvConfig.CONSTANT_SCRIPTVO_LIST_KJBS_MONTH);
